...
// tag::adocuspsteps[]
.Steps in [reference].[usp_PERSIST_RepoObject_referenced_level_T]
[cols="d,15a,d"]
|===
...
[source,sql]
----
CREATE   PROCEDURE [reference].[usp_PERSIST_RepoObject_referenced_level_T]
...
FROM [reference].[RepoObject_referenced_level] AS S
