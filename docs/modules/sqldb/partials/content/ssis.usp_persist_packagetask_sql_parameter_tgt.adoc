...
= ssis.usp_PERSIST_PackageTask_Sql_Parameter_tgt - P
...
// tag::adocuspsteps[]
.Steps in [ssis].[usp_PERSIST_PackageTask_Sql_Parameter_tgt]
[cols="d,15a,d"]
|===
...
----
DELETE T
FROM [ssis].[PackageTask_Sql_Parameter_tgt] AS T
WHERE
NOT EXISTS
(SELECT 1 FROM [ssis].[PackageTask_Sql_Parameter_src] AS S
WHERE
T.[AntoraModule] = S.[AntoraModule]
AND T.[PackageName] = S.[PackageName]
AND T.[TaskPath] = S.[TaskPath]
AND T.[ParameterBindingSequence] = S.[ParameterBindingSequence]
)
...
, T.[ParameterBindingSequence] = S.[ParameterBindingSequence]
, T.[ControlFlowDetailsRowID] = S.[ControlFlowDetailsRowID]
, T.[ParameterBindingParameterName] = S.[ParameterBindingParameterName]
, T.[ResultSetParameterName] = S.[ResultSetParameterName]

FROM [ssis].[PackageTask_Sql_Parameter_tgt] AS T
INNER JOIN [ssis].[PackageTask_Sql_Parameter_src] AS S
ON
T.[AntoraModule] = S.[AntoraModule]
AND T.[PackageName] = S.[PackageName]
AND T.[TaskPath] = S.[TaskPath]
...
WHERE
   T.[ControlFlowDetailsRowID] <> S.[ControlFlowDetailsRowID]
OR T.[ParameterBindingParameterName] <> S.[ParameterBindingParameterName] OR (S.[ParameterBindingParameterName] IS NULL AND NOT T.[ParameterBindingParameterName] IS NULL) OR (NOT S.[ParameterBindingParameterName] IS NULL AND T.[ParameterBindingParameterName] IS NULL)
OR T.[ResultSetParameterName] <> S.[ResultSetParameterName] OR (S.[ResultSetParameterName] IS NULL AND NOT T.[ResultSetParameterName] IS NULL) OR (NOT S.[ResultSetParameterName] IS NULL AND T.[ResultSetParameterName] IS NULL)
...
FROM [ssis].[PackageTask_Sql_Parameter_src] AS S
WHERE
NOT EXISTS
(SELECT 1
FROM [ssis].[PackageTask_Sql_Parameter_tgt] AS T
...
*/
CREATE   PROCEDURE [ssis].[usp_PERSIST_PackageTask_Sql_Parameter_tgt]
...
--
PRINT '[ssis].[usp_PERSIST_PackageTask_Sql_Parameter_tgt]'
--keep the code between logging parameters and "START" unchanged!
--
----START
--
...
--
/*{"ReportUspStep":[{"Number":500,"Name":"delete persistence target missing in source","has_logging":1,"is_condition":0,"is_inactive":0,"is_SubProcedure":0,"log_source_object":"[ssis].[PackageTask_Sql_Parameter_src]","log_target_object":"[ssis].[PackageTask_Sql_Parameter_tgt]","log_flag_InsertUpdateDelete":"D"}]}*/
PRINT CONCAT('usp_id;Number;Parent_Number: ',121,';',500,';',NULL);
...
SET @source_object = '[ssis].[PackageTask_Sql_Parameter_src]'
SET @target_object = '[ssis].[PackageTask_Sql_Parameter_tgt]'
...
/*{"ReportUspStep":[{"Number":600,"Name":"update changed","has_logging":1,"is_condition":0,"is_inactive":0,"is_SubProcedure":0,"log_source_object":"[ssis].[PackageTask_Sql_Parameter_src]","log_target_object":"[ssis].[PackageTask_Sql_Parameter_tgt]","log_flag_InsertUpdateDelete":"U"}]}*/
PRINT CONCAT('usp_id;Number;Parent_Number: ',121,';',600,';',NULL);
...
/*{"ReportUspStep":[{"Number":700,"Name":"insert missing","has_logging":1,"is_condition":0,"is_inactive":0,"is_SubProcedure":0,"log_source_object":"[ssis].[PackageTask_Sql_Parameter_src]","log_target_object":"[ssis].[PackageTask_Sql_Parameter_tgt]","log_flag_InsertUpdateDelete":"I"}]}*/
PRINT CONCAT('usp_id;Number;Parent_Number: ',121,';',700,';',NULL);
