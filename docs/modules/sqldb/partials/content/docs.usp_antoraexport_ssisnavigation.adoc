...
= docs.usp_AntoraExport_SsisNavigation - P
...
// tag::RepoObject_guid[]
8C9A07F7-C23C-EC11-852D-A81E8446D5B0
// end::RepoObject_guid[]
...
// tag::adocuspsteps[]
.Steps in [docs].[usp_AntoraExport_SsisNavigation]
[cols="d,15a,d"]
|===
...
|410
|
*export FROM [docs].[ssis_navlist_package]*
...
Begin
    Set @command = 'bcp "SELECT [nav_list] FROM [docs].[ssis_navlist_package] WHERE '
                   --
                   + '[AntoraModule] = ''' + @AntoraModule + ''''
                   --
                   + '" queryout "'
                   --
                   + config.fs_get_parameter_value ( 'AntoraComponentFolder', '' ) + '\modules\' + @AntoraModule
                   --
                   + '\partials\navlist\' + 'navlist-package' + '.adoc"'
                   --
                   + ' -S ' + @instanceName
                   --
                   + ' -d ' + @databaseName
...
*/
CREATE   PROCEDURE [docs].[usp_AntoraExport_SsisNavigation]
@isTrustedConnection BIT = 1 /* specify whether you are connecting to the SQL instance with a trusted connection (Windows Authentication) or not */
,@userName NVARCHAR(250) = 'loginName' /* If isTrustedConnection is set to 0 then you will need to add username and password for connecting to the SQL Server instance */
,@password NVARCHAR(250) = 'password'
,
...
--
PRINT '[docs].[usp_AntoraExport_SsisNavigation]'
--keep the code between logging parameters and "START" unchanged!
--
----START
--
...
PRINT CONCAT('usp_id;Number;Parent_Number: ',128,';',110,';',NULL);
...
PRINT CONCAT('usp_id;Number;Parent_Number: ',128,';',210,';',NULL);
...
/*{"ReportUspStep":[{"Number":410,"Name":"export FROM [docs].[ssis_navlist_package]","has_logging":1,"is_condition":0,"is_inactive":0,"is_SubProcedure":0,"log_source_object":"[docs].[ssis_navlist_package]","log_flag_InsertUpdateDelete":"u"}]}*/
PRINT CONCAT('usp_id;Number;Parent_Number: ',128,';',410,';',NULL);
...
SET @rows = @@ROWCOUNT
SET @step_id = @step_id + 1
SET @step_name = 'export FROM [docs].[ssis_navlist_package]'
SET @source_object = '[docs].[ssis_navlist_package]'
SET @target_object = NULL
